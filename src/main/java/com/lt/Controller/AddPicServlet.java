...
public class AddPicServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String path=user.getFilePath()+"\\"+uuid;
        //新建一个相册文件夹
        ...
        picdir.setTime(TimeUtil.timeToString());
        picdir.setPicDesc((String) map.get("picDesc"));
        picdir.setPicAddress(path);
        picdir.setUserId(user.getId());
        picdir.setPicCover(filePath);



        //dao层进行数据库操作
        boolean flag= PicdirDao.insertPicdir(picdir);
        if (flag){
            UserDao.updateUserTotal(user.getId());
            httpSession.setAttribute("user",UserDao.queryUser(user.getMail(),user.getPassword()));
